...
class ParticlePhysics(
    ...
    fun createSandParticle(colorType: ColorType, currentTime: Long): SandParticle {
        ...
        val noiseVariation = if (Random.Default.nextFloat() < 0.08f) {
            0.8f + Random.Default.nextFloat() * 0.1f // Range: 0.8 - 0.9 (slightly darker)
        } else {
    ...
    private fun settleParticle(
        ...
        // If sand buildup is disabled and particle reaches bottom, remove it
        if (y >= height - 3) {
            newGrid.setCell(x, y, Cell())// Remove particle
        ...
        newGrid.setCell(x, y, Cell(CellType.SAND, stoppedParticle))

        return MovementResult(
            moved = false,
            newParticle = if (stoppedParticle.isSettled) null else MovingParticle(
                x,
                y,
                stoppedParticle
            ),
    ...
    private fun determineObstacleId(x: Int, y: Int, grid: Array<Array<Cell>>): String? {
        ...
        if (belowY < height) {
            ...
            if (cellBelow.type == CellType.SLIDING_OBSTACLE) {
                return cellBelow.slidingObstacles.firstOrNull()?.id
            }
        ...
        if (currentCell.type == CellType.SLIDING_OBSTACLE) {
            return currentCell.slidingObstacles.firstOrNull()?.id
        }
